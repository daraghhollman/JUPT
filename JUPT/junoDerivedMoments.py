...
def DownloadJadeMoments(dataPath, downloadPath, timeFrame):
    ...
    pathList = [
        ...
        for extension in PathsFromTimeDifference(
            timeFrame[0],
            timeFrame[1],
            "%Y/%Y%j/JAD_L50_HLS_ELC_MOM_ISO_2D_ELECTRONS_%Y%j_V01.CSV",
        )
    ]
    print(f"Downloading Waves files from {downloadPath} to {dataPath}\n")
    for path in tqdm(pathList):
        os.system(
            f"wget -r -q --show-progress -nd -np -nH -N -P {dataPath} -O fileName {path}"
        )
...
def LoadMomentFiles(dataDirectory, timeFrame, downloadPath):
    ...
    filePathsNeeded = PathsFromTimeDifference(
        timeFrame[0],
        timeFrame[1],
        f"{dataDirectory}JAD_L50_HLS_ELC_MOM_ISO_2D_ELECTRONS_%Y%j_V01.CSV",
    )
    ...
    if len(filesToBeDownloaded) > 0:
        ...
        for path in tqdm(filesToBeDownloaded):
            ...
            os.system(
                f"wget -r -q -nd -nv -np -nH -N -P {dataDirectory} -O {fileName} {downloadPath}{fileLinks[linkIndex]}"
            )
    ...
    for filePath in tqdm(filePaths):
        fileInfoList.append(
            pandas.read_csv(
                ...
                names=[
                    "UTC",
                    "SOURCE_JADE_LEVEL5_DEF_VERSION_INPUT",
...
def PullJadeMoments(dataDirectory, timeFrame, downloadNewData):
    ...
    if downloadNewData:
        ...
        DownloadJadeMoments(
            dataDirectory,
            "https://pds-ppi.igpp.ucla.edu/ditdos/download?id=pds://PPI/JNO-J-JAD-5-MOMENTS-V1.0/DATA/",
            timeFrame,
        )

    filesWithInfo = LoadMomentFiles(
        dataDirectory,
        timeFrame,
        "https://pds-ppi.igpp.ucla.edu/ditdos/download?id=pds://PPI/JNO-J-JAD-5-MOMENTS-V1.0/DATA/",
    )
...
def PlotDensity(
    ...
    for index, row in tqdm(totalFile.iterrows(), total=len(totalFile)):
        ...
        if time >= timeFrameStart and not foundStart:
            sliceStart = index
            foundStart = True

        if time >= timeFrameEnd and not foundEnd:
            sliceEnd = index
            foundEnd = True
    ...
    if not plotEphemeris:
        ...
        ax = junoEphemeris.PlotEphemeris(
            ax, timesDatetime64, timeFrame, labels=ephemerisLabels
        )
