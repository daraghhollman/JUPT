...
def DownloadJadeData_wget(dataDirectory, downloadPath, timeFrame, hiRes=False):
    ...
    if hiRes:
        binaryPathList = [
            ...
            for extension in PathsFromTimeDifference(
                timeFrame[0],
                timeFrame[1],
                "%Y/%Y%j/ION_TOF/JAD_L50_HLS_ION_TOF_DEF_%Y%j_V01.DAT",
            )
        ]
        labelPathList = [
            f"{downloadPath}{extension}"
            for extension in PathsFromTimeDifference(
                timeFrame[0],
                timeFrame[1],
                "%Y/%Y%j/ION_TOF/JAD_L50_HLS_ION_TOF_DEF_%Y%j_V01.LBL",
            )
        ]
    else:
        binaryPathList = [
            ...
            for extension in PathsFromTimeDifference(
                timeFrame[0],
                timeFrame[1],
                "%Y/%Y%j/ION_TOF/JAD_L50_LRS_ION_TOF_DEF_%Y%j_V01.DAT",
            )
        ]
        labelPathList = [
            f"{downloadPath}{extension}"
            for extension in PathsFromTimeDifference(
                timeFrame[0],
                timeFrame[1],
                "%Y/%Y%j/ION_TOF/JAD_L50_LRS_ION_TOF_DEF_%Y%j_V01.LBL",
            )
        ]

    print(
        f"Downloading {len(labelPathList)} JADE label file(s) from {downloadPath} to {dataDirectory}\n"
    )
    ...
    print(
        f"Downloading {len(binaryPathList)} JADE binary file(s) from {downloadPath} to {dataDirectory}\n"
    )
    ...
    for fileExtension in ["DAT", "LBL"]:
        ...
        if hiRes:
            filePathsNeeded = PathsFromTimeDifference(
                timeFrame[0],
                timeFrame[1],
                f"{dataDirectory}JAD_L50_HLS_ION_TOF_DEF_%Y%j_V01.{fileExtension}",
            )
        else:
            filePathsNeeded = PathsFromTimeDifference(
                timeFrame[0],
                timeFrame[1],
                f"{dataDirectory}JAD_L50_LRS_ION_TOF_DEF_%Y%j_V01.{fileExtension}",
            )
        ...
        if hiRes:
            fileLinks = PathsFromTimeDifference(
                timeFrame[0],
                timeFrame[1],
                f"%Y/%Y%j/ION_TOF/JAD_L50_HLS_ION_TOF_DEF_%Y%j_V01.{fileExtension}",
            )
        else:
            fileLinks = PathsFromTimeDifference(
                timeFrame[0],
                timeFrame[1],
                f"%Y/%Y%j/ION_TOF/JAD_L50_LRS_ION_TOF_DEF_%Y%j_V01.{fileExtension}",
            )
...
def PlotData(
    ...
    if ionTimeOfFlightRange != []:
        ...
        timeOfFlightBandIndices = np.where(
    ...
    if plotIonEnergy:
        image = ax.pcolormesh(
            index_array,
            [el for el in filesWithInfo[0]["energy scale"]],
            newDataArray,
            cmap=colourmap,
            norm=colors.LogNorm(),
        )
